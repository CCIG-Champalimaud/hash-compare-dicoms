 ...
 */
async function writeFilePathsToTemp(dir, deepMode, writeStream) {
  let entries
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await writeFilePathsToTemp(fullPath, deepMode, writeStream)
 ...
 * @returns {null} 
 */
async function saveAllFilePaths(dirPath, deepMode, tempFilePath) {
  
  const writeStream = fs.createWriteStream(tempFilePath, { flags: 'a' })

  await writeFilePathsToTemp(dirPath, deepMode, writeStream)
